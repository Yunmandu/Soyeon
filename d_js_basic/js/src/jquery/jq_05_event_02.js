...
/* 
   ...
   mytop.on('mouseenter', function () { })
        .on('mouseleave', function () { }); //매소드 체인을 이용한기능
  */     
   ...
   * transformm, transitiom, animation, borderRadius 등
   * 흔히말하는 css3기능은 동작하지 않는다.
   * 또한, backgroundColor, color 등 색상관련된 기능은
   * jquery-uI 에서 사용되는 기능
  ...
  const conBox = $('#contentBox');
   ...
   win.on( 'scroll', function(){
    ...
    //  (myscroll > 150) ? conBox.css({'backgroundColor':'#aca'}) : conBox.css({
    //    'backgroundColor': '#fca'});

   });
 
   // 크기값이 변경되었을때 사용하는기능(가로,세로 일부기능 제한 두고 있음)
